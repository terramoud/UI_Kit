 ...
 *   <div class="dropdown-facilities__quantity-block">
...
jQuery('select.dropdown-facilities').each(function () {
  /**
   * Namespace
   */
  var cloneThis = this;
  var dropdownFacilities = {
    widgetClass: '.dropdown-facilities',
  ...
  $('#'+dropdownFacilities.id).multiselect({
    ...
    create: function(argument) {
      ...
      $(this).next().find('.ui-multiselect-open').addClass('dropdown-facilities__icon');
      $(this).next().find('span:last').addClass('dropdown-facilities__widget-text');
    },
    ...
    selectedText: function(numChecked, numTotal, checkedItems){
      value = this.$inputs.filter(':checked').map(function() {
         ...
         */
        var valInputNumber = $(this).siblings('.dropdown-facilities__quantity-block').children('input').val();
        if (!valInputNumber) {
          valInputNumber = "2 ";
        } else {
          valInputNumber += " ";
  ...
  /**
   * This are adding styles for widget dropdown-facilities
   */
  ...
  $('.ui-multiselect-checkboxes label[for *= "' + dropdownFacilities.id + '"]').each(function(index, el) {
    $(this).append('<div class="dropdown-facilities__quantity-block">' +
      '<button class="dropdown-facilities__quantity-icon-minus dropdown-facilities__quantity-icon-minus_no-active"> - </button>' +
      '<input class="dropdown-facilities__quantity-num" type="text" placeholder="0" />' +
      '<button class="dropdown-facilities__quantity-icon-plus"> + </button>' +
      '</div>');
  });
  ...
  $('.ui-multiselect-checkboxes input[type=checkbox]').each(function(index) {
    $(this).siblings('.dropdown-facilities__quantity-block').children('input').attr('name', $(this).val());
    $(this).siblings('.dropdown-facilities__quantity-block').children('input').val('0'); // It is fixed the bag that happenes after was being clicked on this input field
  });
   ...
   */
  $("label[data-id=" + dropdownFacilities.id + "] .dropdown-facilities__quantity-icon-plus").click(function(eventObject){
    ...
    $(this).siblings('.dropdown-facilities__quantity-icon-minus').removeClass('dropdown-facilities__quantity-icon-ninus_no-active');
    $(this).parent('.dropdown-facilities__quantity-block').siblings('input').prop('checked', 'checked');


    /**
     * This is changes text in 'widget field' the by adding number of selected elements using a regular expression
     */
    var widgetId = $(this).parent('.dropdown-facilities__quantity-block').siblings('input').attr('id');
    ...
    let modifiedText = '';
    let numFacilities = 0;
    jQuery("label[data-id=" + dropdownFacilities.id + "] .dropdown-facilities__quantity-num").each(function () {
   ...
   * This events are adding count of selected items in multiselect widget
   */
  $("label[data-id=" + dropdownFacilities.id + "] .dropdown-facilities__quantity-icon-minus").click(function(eventObject){
    ...
    if (targetMinus.val() > 0) {
       ...
       */
      if (targetMinus.val() <= 0) {
        $(this).addClass('dropdown-facilities__quantity-icon-minus_no-active');
        $(this).closest('.dropdown-facilities__quantity-block').siblings('input').prop('checked', false);
      }
